...
- 오프라인 시향 노트 저장 응답 시간 0ms (네트워크 미의존)
- 회원 탈퇴 후 Firestore 잔류 데이터 0건 달성
- Fragella API 74,000+개 향수 데이터베이스 연동 및 국내 브랜드 한국어 매핑 테이블 직접 구축
...
각 노트에 `syncStatus`를 부여해 네트워크 없이도 즉시 저장·조회하고, 연결 복구 시 자동 동기화합니다.

```swift
// 오프라인 우선 저장 상태 관리
enum SyncStatus: String {
    case pending        // 로컬 저장 완료, 동기화 대기
    case synced         // Firestore 동기화 완료
    case failed         // 동기화 실패
    case pendingDelete  // 삭제 대기 (오프라인 상태)
}
```
...
```swift
// 탈퇴 순서 보장 — Firestore 삭제 완료 후 Auth 삭제
func deleteAccount(uid: String) async throws {
    try await firestoreRepository.deleteAllUserData(uid: uid) // 1. Firestore 먼저
    try await authRepository.deleteAccount()                  // 2. Auth 나중에
}
```
...
- 회원 탈퇴 순서 보장 로직부터 XCTest 단위 테스트 추가 (Repository 프로토콜 추상화로 Mock 구현체만 만들면 즉시 적용 가능한 구조)
